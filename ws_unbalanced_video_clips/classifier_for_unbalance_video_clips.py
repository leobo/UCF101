...
def nn_classifier(train_input_path, eval_input_path, params):
    with tf.device('/cpu:1'):
        ...
        input_rgb = tf.transpose(input_rgb, [0, 1, 3, 2])
        input_rgb = tf.reshape(input_rgb, [-1, 2, 2048, 1])
        input_flow = tf.transpose(input_flow, [0, 1, 3, 2])
        input_flow = tf.reshape(input_flow, [-1, 4, 2048, 1])
        input_data = tf.concat([input_rgb, input_flow], axis=1)
        # # input_data = tf.layers.batch_normalization(input_data, axis=2, training=(mode == tf.estimator.ModeKeys.TRAIN),
        # #                                            name='input_bn')

        conv1 = tf.layers.conv2d(inputs=input_data, filters=32,
        ...
        # conv1_r = tf.layers.conv2d(inputs=conv_act1, filters=16,
        #                            kernel_initializer=tf.contrib.layers.xavier_initializer(),
        #                            use_bias=True, kernel_size=[1, 1], padding="valid", activation=None,
        #                            name='conv1_r')
        ...
        conv3 = tf.layers.conv2d(inputs=conv_act2, filters=8,
                                 kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                 use_bias=True, kernel_size=[4, 1], padding="valid", activation=None,
                                 name='conv3')
        ...
        flat = tf.reshape(conv_act3, [-1, 3 * 2048 * 8], name='flat')

        fc1 = tf.layers.dense(inputs=flat, units=512, activation=None, name='fc1')
        ...
        fc2 = tf.layers.dense(inputs=dropoutfc1, units=512, activation=None, name='fc2')
