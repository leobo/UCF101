...
def _parse_fn(example):
    ...
    rgb = tf.reshape(rgb, [1, 2048, 2])
    flow = tf.decode_raw(parsed["flow"], tf.float32)
    flow = tf.reshape(flow, [1, 2048, 2])
...
def input_fn(train_input_path, eval_input_path):
    ...
    train_dataset = train_dataset.shuffle(buffer_size=9537 * 5)
...
def nn_classifier(train_input_path, eval_input_path, params):
    with tf.device('/cpu:1'):
        ...
        conv1_f = tf.layers.conv2d(inputs=input_flow, filters=8,
                                   kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                   use_bias=True, kernel_size=[1, 1], padding="same", activation=None,
                                   name='conv1_f')
        conv_bn1_f = tf.layers.batch_normalization(conv1_f, training=(mode == tf.estimator.ModeKeys.TRAIN),
                                                   name='conv_bn1_f')
        conv_act1_f = tf.nn.selu(conv_bn1_f, name="conv_act1_f")

        conv2_f = tf.layers.conv2d(inputs=conv_act1_f, filters=4,
                                   kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                   use_bias=True, kernel_size=[1, 1], padding="valid", activation=None,
                                   name='conv2_f')
        conv_bn2_f = tf.layers.batch_normalization(conv2_f, training=(mode == tf.estimator.ModeKeys.TRAIN),
                                                   name='conv_bn2_f')
        conv_act2_f = tf.nn.selu(conv_bn2_f, name="conv_act2_f")

        flat_f = tf.reshape(conv_act2_f, [-1, 1 * 2048 * 4], name='flat_f')

        fc1_f = tf.layers.dense(inputs=flat_f, units=1024, activation=None, name='fc1_f')
        fc_bn1_f = tf.layers.batch_normalization(inputs=fc1_f, training=(mode == tf.estimator.ModeKeys.TRAIN),
                                                 name='fc_bn1_f')
        fc_act1_f = tf.nn.selu(fc_bn1_f, name="fc_act1_f")
        dropoutfc1_f = tf.layers.dropout(
            inputs=fc_act1_f, rate=dropout_rate, training=(mode == tf.estimator.ModeKeys.TRAIN), name='dropoutfc1')

        # Logits Layer
        logits_f = tf.layers.dense(inputs=dropoutfc1_f, units=num_unique_classes, name='logits_f')

        conv1_r = tf.layers.conv2d(inputs=input_rgb, filters=8,
                                   kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                   use_bias=True, kernel_size=[1, 1], padding="valid", activation=None,
                                   name='conv1_r')
        conv_bn1_r = tf.layers.batch_normalization(conv1_r, training=(mode == tf.estimator.ModeKeys.TRAIN),
                                                   name='conv_bn1_r')
        conv_act1_r = tf.nn.selu(conv_bn1_r, name="conv_act1_r")

        conv2_r = tf.layers.conv2d(inputs=tf.multiply(conv_act1_r, conv_act1_f), filters=4,
                                   kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                   use_bias=True, kernel_size=[1, 1], padding="valid", activation=None,
                                   name='conv2_r')
        conv_bn2_r = tf.layers.batch_normalization(conv2_r, training=(mode == tf.estimator.ModeKeys.TRAIN),
                                                   name='conv_bn2_r')
        conv_act2_r = tf.nn.selu(conv_bn2_r, name="conv_act2_r")

        flat_r = tf.reshape(tf.multiply(conv_act2_r, conv_act2_f), [-1, 1 * 2048 * 4], name='flat_r')

        fc1_r = tf.layers.dense(inputs=flat_r, units=1024, activation=None, name='fc1_r')
        fc_bn1_r = tf.layers.batch_normalization(inputs=fc1_r, training=(mode == tf.estimator.ModeKeys.TRAIN),
                                                 name='fc_bn1_r')
        fc_act1_r = tf.nn.selu(fc_bn1_r, name="fc_act1_r")
        dropoutfc1_r = tf.layers.dropout(
            inputs=fc_act1_r, rate=dropout_rate, training=(mode == tf.estimator.ModeKeys.TRAIN), name='dropoutfc1_r')

        logits_r = tf.layers.dense(inputs=dropoutfc1_r, units=num_unique_classes, name='logits_r')

        logits = tf.reduce_mean(tf.stack([logits_r, logits_f], axis=1), axis=1)
...
def classify(train_records, test_records, num_train_samples, num_test_samples, num_test_samples_per_video):
    ...
    #     for j in range(1, int(math.ceil(num_test_samples / num_samples_per_test_video)) + 2):
    ...
    with tf.device('/cpu:1'):
        params = {"rgb_feature_shape": (1, 2048, 2),
                  "flow_feature_shape": (1, 2048, 2)}
