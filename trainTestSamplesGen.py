...
class TrainTestSampleGen(object):
    ...
    def __init__(self, ucf_path, hmdb_path):
        if ucf_path != '':
            self.ucf_train_data_label, self.ucf_test_data_label = self.ucf_read_train_test_split(ucf_path)
        if hmdb_path != '':
            self.hmdb_train_data_label, self.hmdb_test_data_label = self.hmdb_read_train_test_split(hmdb_path)
