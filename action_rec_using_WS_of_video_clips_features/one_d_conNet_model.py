...
def nn_classifier_estimator(features, labels, mode, params):
    with tf.device('/device:GPU:1'):
        ...
        conv2 = tf.layers.conv2d(inputs=input_layer, filters=16,
                                 kernel_initializer=tf.contrib.layers.xavier_initializer(),
        ...
        conv3 = tf.layers.conv2d(inputs=pool2, filters=8, kernel_initializer=tf.contrib.layers.xavier_initializer(),
        ...
        # pool3 = tf.layers.average_pooling3d(inputs=conv_act3, pool_size=[1, 2, 1], strides=[1, 2, 1])

        flat = tf.reshape(conv_act3, [-1, 8 * 4096], name='flat')

        fc1 = tf.layers.dense(inputs=flat, units=4096, activation=None, name='fc1')
        ...
        # Logits Layer
        logits = tf.layers.dense(inputs=dropoutfc3, units=num_unique_classes, name='logits')
